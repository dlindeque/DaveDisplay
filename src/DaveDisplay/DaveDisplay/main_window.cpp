...
namespace dave
{
        ...
        case WM_COMMAND:
            {
                ...
                {
                case IDM_ABOUT:
                    dave::about_dialog::try_show(hWnd);
                    break;
                case IDM_EXIT:
                    DestroyWindow(hWnd);
                    break;
    ...
    auto main_window::try_register_class() -> bool
    {
        ...
        wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_DAVEDISPLAY);
    ...
    auto main_window::show_canvas_window() -> void
    {
        if (!configuration_manager::try_show_canvas_window()) {
            MessageBoxW(_wnd, dave::get_last_error::get_error_msg(L"Unknown error").c_str(), L"Failure creating the canvas window", MB_OK);
        }
    }
